...
btnClick('#searchInvoiceBatch', (e) => {
	...
	.then((doc) => {
		...
		.then((doc) => {
			...
			return searchPendingInvoice(resident);
		})
...
function searchResident(list) {
	return new Promise((resolve, reject) => {
		console.log(list);
		mongoDbObj.residents.find({id: {$in: list}}).toArray((err, doc) => {
			if (err) {
				reject(err);
			} else {
...
function searchPendingInvoice(resident) {
	...
	pendingInvoices.shift(resident.invoice.id);
	resident.otherInvoices = [];
	mongoDbObj.invoices.find({id: {$in: pendingInvoices}}, (err, doc) => {
		console.log(doc);
		if (err) {
			return Promise.reject(err);
		}
		resident.otherInvoices.push(doc);
